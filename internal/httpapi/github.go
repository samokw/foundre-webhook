...
type PreviewHandler interface {
	Handle(domain.PreviewRequest) error
	Host(domain.PreviewRequest) (string, error)
}

func GithubWebhook(ph PreviewHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if host, err := ph.Host(pr); err == nil {
			log.Printf("preview host=%s", host)
		}
		if err := ph.Handle(pr); err != nil {
